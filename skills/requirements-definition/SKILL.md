...
If the subsystem's class has no rows for §4.1 in the matrix, this skill has nothing to record — say so and don't run the script at all (calling it with an id absent from the matrix raises `KeyError`). Check the matrix first: Class E carries no §4.1 rows in Appendix C, so this skill has nothing to record for a Class E subsystem.
