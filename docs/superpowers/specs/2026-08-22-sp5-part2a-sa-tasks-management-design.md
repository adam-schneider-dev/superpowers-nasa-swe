...
- **`data/sa-task-catalog.yaml`** — new, 45 rows for this sub-spec (grows to 103 across Parts 2a-2c). Row shape: `swe_id` (matches `swe-catalog.yaml`'s existing id, e.g. `"SWE-033"`), `section` (e.g. `"3.1.2"`), `tasks` (list of strings, the row's numbered SA/safety tasks, paraphrased — same "cite by id/section, don't reproduce source text verbatim" convention as every other catalog in this fork). SWE-065's four lettered sub-items become four rows with `swe_id: "SWE-065a"` through `"SWE-065d"`, each carrying its own `section` (`"4.5.2a"` etc. — out of scope for Ch.3, noted here only so Parts 2b's spec doesn't have to re-derive this from the PDF).
